...
void Test1() {
    ...
    struct DeletionSpy {
        ...
        DeletionSpy& operator=(const DeletionSpy& rhs) noexcept {
            if (this != &rhs) {
                auto rhs_copy(rhs);
                swap(instance_counter_ptr_, rhs_copy.instance_counter_ptr_);
            }
...
void Test3() {
    ...
    // Обмен содержимого списков
    {
        ...
        {
            // В отсутствие пользовательской перегрузки будет вызвана функция swap, которая
            // выполнит обмен через создание временной копии
            swap(first, second);

            // Убеждаемся, что используется не swap, а пользовательская перегрузка
    ...
    // Инициализация списка при помощи initializer_list
    {
        SingleLinkedList<int> list{1, 2, 3, 4, 5};
        assert(list.GetSize() == 5);
        assert(!list.IsEmpty());
        assert(equal(list.begin(), list.end(), begin({1, 2, 3, 4, 5})));
    }
